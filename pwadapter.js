...
(function () {
    ...
    function process(manifest, urlFactory) {
        ...
        const icons = allIcons.filter(
            (icon) => icon.purpose.indexOf("any") > -1
        );
        const maskable = allIcons.filter(
            (icon) => icon.purpose.indexOf("maskable") > -1
        );

        // Mod by Giuseppe Rossi

        const transparentIcons = (icons.length > 0 ? icons : maskable)
            .map((icon) => {
                const attr = {
                    rel: "icon",
        ...
        const appleTouchIcons = (maskable.length > 0 ? maskable : icons)
            .map((icon) => {
                ...
                return push(
                    "link",
                    attr,
                    '[rel="apple-touch-icon"]' + querySuffix
                );
            })
            .filter(Boolean);
        ...
        const backgroundColor =
            /** @type {string} */ (manifest["background_color"]) ||
            defaultSplashColor;
        ...
        function splashFor(width, height, orientation, icon) {
            ...
            const title =
                manifest["name"] || manifest["short_name"] || document.title;
            const measure = ctx.measureText(title);
            const textHeight =
                measure.actualBoundingBoxAscent || defaultSplashTextSize;
        ...
        function appendSplash(orientation, data) {
            const generatedSplash = /** @type {!HTMLLinkElement} */ (
                document.createElement("link")
            );
            generatedSplash.setAttribute("rel", "apple-touch-startup-image");
            generatedSplash.setAttribute(
                "media",
        ...
        if (!debug && rendered) {
            try {
                const prev = /** @type {!Object<string, string>} */ (
                    JSON.parse(rendered)
                );
            ...
            img.onload = () => {
                ...
                renderBothSplash(img, () => {
                    ...
                    const redrawn =
                        ...
                        updateTransparent(img, backgroundColor);
                    if (redrawn) {
                        icon.href = redrawn;
